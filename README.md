...
So I set out to implement a system similar to Redux by accumulating regular DOM events into a state object. I'm using RxJS to merge all the events I'm interested in into an Observable, then running an accumulator function over them using the `scan()` function. `scan()` is similar to `reduce()` but it emits the results each time. So after each event I get a new state object which I pass to a Virtual-DOM render function.
